...
`PATCH /auth/profile` accepts partial updates for:

- `name`
- `phone`
- `timezone`
- `defaultShippingAddress` (`name`, `line1`, optional `line2`, `city`, `state`, `postalCode`, `country`)

Patch semantics:

- Omitted fields are left unchanged.
- Explicit `null` for `phone`, `timezone`, or `defaultShippingAddress` clears the field.
- `name` is updated only when a non-empty string is provided.

`GET /auth/profile` and `PATCH /auth/profile` both return `{ "user": AuthUser }` including computed `profileComplete`.
...
`GET /support/tickets` query params:
...
`PATCH /support/tickets/{ticket_id}` request:

```json
{
  "status": "resolved",
  "note": "Issue resolved from account support tab"
}
```
...
- `stream_start.payload`: `{ "streamId": string, "agent": string }`
- `stream_delta.payload`: `{ "streamId": string, "delta": string }`
- `stream_end.payload`: `{ "streamId": string }`

`streamId` is stable across `stream_start`, `stream_delta`, and `stream_end` for a
single streamed assistant response.
